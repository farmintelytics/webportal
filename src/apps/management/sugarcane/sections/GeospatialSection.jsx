...
import { Layers, Satellite, Map as MapIcon, Truck, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-[calc(100vh-220px)] animate-in fade-in slide-in-from-bottom-4 bg-gray-50/50 p-10">
       ...
       <div className="lg:w-1/4 flex flex-col gap-8 overflow-y-auto">
          <SimpleCard title="Mapping Layers" icon={<Layers size={20} />}>
             <div className="space-y-3">
                {[
                  { id: 'satellite', label: 'Satellite View', icon: <Satellite size={16}/> },
                  { id: 'plots', label: 'Cane Blocks', icon: <MapIcon size={16}/> },
                  { id: 'logistics', label: 'Truck Routing', icon: <Truck size={16}/> },
                ].map(layer => (
                  <button 
                    key={layer.id} 
                    onClick={() => setActiveLayer(layer.id)}
                    className={`w-full flex items-center justify-between p-5 rounded-2xl border transition-all duration-300 ${
                      activeLayer === layer.id ? 'bg-green-600 border-green-600 text-white shadow-lg' : 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50'
                    }`}
                  >
