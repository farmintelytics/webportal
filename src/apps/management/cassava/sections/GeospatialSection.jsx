          ...
          <SimpleCard title="Mapping Intelligence" icon={<Layers size={20} />}>
             <div className="space-y-3">
                {[
                  ...
                  { id: 'plots', label: 'Cassava Blocks', icon: <MapIcon size={16}/> },
                  { id: 'health', label: 'Tuber Health', icon: <Activity size={16}/> },
                ].map(layer => (
                  <button 
                    key={layer.id} 
                    onClick={() => setActiveLayer(layer.id)}
                    className={`w-full flex items-center justify-between p-5 rounded-2xl border transition-all duration-300 ${
                      activeLayer === layer.id ? 'bg-lime-600 border-lime-600 text-white shadow-lg' : 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50'
                    }`}
                  >
