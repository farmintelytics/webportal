...
const EstimatorSustainability = ({ onBack }) => {
   const [activeTab, setActiveTab] = useState('overview');
   const [activeLayer, setActiveLayer] = useState('Carbon Density');
   const [userName] = useState('Admin Manager');

   const carbonData = [
      { id: 'EST-MAI-01', area: '10.0 HA', carbonStock: '45.2 tCO2e', health: 'N/A', landUse: 'Maize Mono', status: 'Projection' },
      { id: 'EST-RIC-02', area: '25.0 HA', carbonStock: '112.8 tCO2e', health: 'N/A', landUse: 'Rice Flooded', status: 'Projection' },
      { id: 'EST-FFB-03', area: '100.0 HA', carbonStock: '1,240 tCO2e', health: 'N/A', landUse: 'Oil Palm', status: 'Projection' },
   ];

   const columns = [
      { key: 'id', label: 'Estimation ID' },
      { key: 'area', label: 'Land Area' },
      { key: 'landUse', label: 'Proposed Crop' },
      { key: 'carbonStock', label: 'Est. Carbon Stock', render: (val) => <span className="text-[13px] font-bold text-sky-600">{val}</span> },
      ...
      <div className="h-screen flex flex-col bg-gray-50 text-gray-900 overflow-hidden font-sans antialiased">
         <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-10 z-[1100] shadow-sm">
            <div className="flex items-center gap-8">
               ...
               <div className="w-11 h-11 bg-sky-600 rounded-xl flex items-center justify-center p-2 shadow-lg ring-4 ring-sky-50">
...
export default EstimatorSustainability;
