...
const center: [number, number] = [6.35, 5.68]; // Near Edo State, Nigeria

function mkPoly(cx: number, cy: number, size: number): [number, number][] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    pts.push([+(cx + Math.sin(a) * r).toFixed(6), +(cy + Math.cos(a) * r * 1.2).toFixed(6)]);
  }
  return pts;
}
...
const defs = [
  { id: "B01", estate: "Ondo Alpha", year: 2022, area: 22, lat: center[0] + 0.012, lng: center[1] - 0.018 },
  { id: "B02", estate: "Ondo Alpha", year: 2022, area: 18, lat: center[0] + 0.015, lng: center[1] - 0.005 },
  { id: "B03", estate: "Ondo Alpha", year: 2017, area: 30, lat: center[0] + 0.008, lng: center[1] + 0.008 },
  { id: "B04", estate: "Ondo Alpha", year: 2017, area: 28, lat: center[0] - 0.005, lng: center[1] - 0.015 },
  { id: "B05", estate: "Ondo Alpha", year: 2012, area: 35, lat: center[0] - 0.012, lng: center[1] - 0.002 },
  { id: "B06", estate: "Ondo Alpha", year: 2012, area: 40, lat: center[0] - 0.018, lng: center[1] - 0.012 },
  { id: "B07", estate: "Cross River Beta", year: 2009, area: 25, lat: center[0] + 0.025, lng: center[1] + 0.025 },
  { id: "B08", estate: "Cross River Beta", year: 2009, area: 33, lat: center[0] + 0.028, lng: center[1] + 0.038 },
  { id: "B09", estate: "Cross River Beta", year: 2003, area: 38, lat: center[0] + 0.018, lng: center[1] + 0.035 },
  { id: "B10", estate: "Cross River Beta", year: 2003, area: 42, lat: center[0] + 0.032, lng: center[1] + 0.045 },
  { id: "B11", estate: "Cross River Beta", year: 1999, area: 30, lat: center[0] + 0.040, lng: center[1] + 0.032 },
  { id: "B12", estate: "Cross River Beta", year: 1999, area: 28, lat: center[0] + 0.038, lng: center[1] + 0.022 },
];
...
export const blocks: Block[] = defs.map((d, i) => {
  ...
  const size = Math.sqrt(d.area) * 0.001;
  return {
    id: d.id,
    estate: d.estate,
    plantingYear: d.year,
    ...
    overdue: d.id === "B07",
    polygon: mkPoly(d.lat, d.lng, size),
