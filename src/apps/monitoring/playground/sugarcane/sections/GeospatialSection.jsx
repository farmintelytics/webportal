    ...
    <div className="flex-1 flex flex-col relative animate-in fade-in duration-500 h-full">
       ...
       <div className="flex-1 bg-gray-200 relative overflow-hidden">
          <MapContainer center={[6.5244, 3.3792]} zoom={13} style={{ height: '100%', width: '100%' }} zoomControl={false}>
             ...
             {plots.map(plot => (
                <Polygon key={plot.id} positions={[[6.5244, 3.3792], [6.5264, 3.3792], [6.5264, 3.3812], [6.5244, 3.3812]]} pathOptions={{ color: '#PURPLE', fillOpacity: 0.2 }} eventHandlers={{ click: () => setSelectedPlot(plot) }} />
             ))}
             ...
             <div className="bg-gray-900 text-white shadow-2xl rounded-none flex flex-col pointer-events-auto overflow-hidden border border-gray-800">
                <div className="p-3 flex items-center justify-between border-b border-gray-800 bg-black/50">
                   ...
                   <button onClick={() => setShowLayerList(!showLayerList)} className={`p-1.5 hover:bg-gray-800 rounded-none transition-all ${!showLayerList && 'w-full flex justify-center'}`}>
                      {showLayerList ? <ChevronRight size={14} /> : <Layers size={20} className="text-purple-500" />}
                   </button>
                </div>
                {showLayerList && (
                      ...
                      {layers.map(layer => (
                         <div key={layer.id} className="space-y-2">
                            <button onClick={() => setLayers(layers.map(l => l.id === layer.id ? { ...l, active: !l.active } : l))} className="flex items-center gap-2 group text-left w-full">
                               <div className={`w-3 h-3 rounded-none border transition-all shrink-0 flex items-center justify-center ${layer.active ? 'bg-purple-500 border-purple-500' : 'border-gray-600'}`}>{layer.active && <CheckCircle2 size={8} className="text-black" />}</div>
                               <span className={`text-[8px] font-bold uppercase tracking-widest leading-tight ${layer.active ? 'text-purple-400' : 'text-gray-500'}`}>{layer.label}</span>
                            </button>
                            {layer.active && (
             ...
             {selectedPlot && showLayerList && (
                <div className="mt-4 bg-gray-900 border border-gray-800 rounded-none p-5 shadow-2xl animate-in slide-in-from-right-10 duration-500 pointer-events-auto flex flex-col gap-4">
                   <div className="flex justify-between items-start">
                      <div>
                         <div className="text-[8px] font-bold text-purple-500 uppercase tracking-[0.2em] mb-1 leading-none italic">{config.drillDownType}</div>
                   ...
                   <div className="bg-black/50 p-4 rounded-none border border-gray-800">
                      ...
                      <p className="text-[11px] font-bold italic leading-tight text-purple-100">"{selectedPlot.layman}"</p>
                   </div>
                      ...
                      <div className="h-24 bg-black rounded-none p-2 border border-gray-800">
                         <Line
                            data={{
                               ...
                               datasets: [{ fill: true, data: selectedPlot.history, borderColor: '#a855f7', backgroundColor: 'rgba(168, 85, 247, 0.1)', tension: 0.4, pointRadius: 0 }]
                            }}
                   ...
                   <div className="flex items-center gap-2 pt-3 border-t border-gray-800 space-y-2">
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-widest text-purple-300 italic leading-tight">{selectedPlot.advice}</span>
                   </div>
                </div>
             )}
          </div>
