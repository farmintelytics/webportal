...
const CONFIG = {
   primaryColor: 'cyan',
   theme: 'Latex / Bark Restoration Focused',
   indices: [
      ...
      { id: 'LST', label: 'Thermal Stress (LST)', legend: 'Cool - Hot', color: 'from-blue-200 to-red-600', active: false, opacity: 60 },
   ],
   kpis: [
      { label: 'Bark Recovery', value: 'OPTIMAL', unit: 'RATE', icon: <Activity /> },
      { label: 'Canopy Vitality', value: '0.68', unit: 'EVI', icon: <Trees /> },
      { label: 'Latex Forecast', value: 'HIGH', unit: 'POTENTIAL', icon: <Droplets /> },
   ],
   layman: {
      health: 'Canopy vitality is high. Optimal conditions for latex production.',
      stress: 'Thermal stress detected. Bark recovery may be slowed.',
      yield: 'Current physiological state predicts 1.4 t/ha dry rubber.'
   },
   drillDownType: 'Bark Anomaly'
};

const RubberMonitoring = ({ onBack }) => {
   ...
   const plots = [
      { id: `B-RUB-01`, area: '4.2 HA', health: '98%', status: 'Healthy', ndvi: 0.72, layman: CONFIG.layman.health, advice: 'No action needed.', history: [0.3, 0.45, 0.58, 0.72, 0.75, 0.72] },
      { id: `B-RUB-04`, area: '2.8 HA', health: '35%', status: 'Stressed', ndvi: 0.35, layman: CONFIG.layman.stress, advice: 'Inspect and apply remediation.', history: [0.3, 0.32, 0.40, 0.38, 0.35, 0.33] },
   ];
                  ...
                  <div className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-8 z-[1010]">
                     <div className="flex items-center gap-6 flex-1 max-w-4xl">
                        <div className="relative flex-1">
                           ...
                           <input type="text" placeholder="Locate Rubber Plot..." className="w-full bg-gray-50 border border-gray-100 rounded-xl py-2.5 pl-12 pr-4 text-[12px] font-bold outline-none focus:border-emerald-500 transition-all" />
                        </div>
                     </div>
            ...
            return (
               <div className="p-10 space-y-10 animate-in fade-in duration-500 overflow-y-auto h-full bg-gray-50/50">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                     <SimpleCard title="Rubber Growth Trajectory" icon={<TrendingUp size={20} />}>
      ...
      <div className="h-screen flex flex-col bg-gray-50 text-gray-900 overflow-hidden font-sans antialiased">
         <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-10 z-[1100] shadow-sm">
            <div className="flex items-center gap-8">
               ...
               <div>
                  <h1 className="text-lg font-black tracking-tighter leading-none uppercase italic">Rubber <span className="text-gray-400 font-medium ml-1">Monitoring & RS</span></h1>
...
export default RubberMonitoring;
