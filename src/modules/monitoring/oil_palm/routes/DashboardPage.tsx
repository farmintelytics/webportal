...
// NDVI by age class ÔÇö average per month
const ndviByAgeData = (() => {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <KPI label="Total Planted Area" value={`${liveArea} ha`} sub={cropSummary ? `${livePlots} monitored plots` : "12 active blocks ┬À 2 estates"}/>
        <KPI label="Active Plots" value={livePlots} sub={cropSummary ? `Scene: ${cropSummary.scene_date}` : "2 immature ┬À 8 producing ┬À 2 replant"}/>
      ...
      <div className="flex flex-wrap items-center gap-2 bg-card border border-border rounded-lg p-3">
        ...
        {["Estate: All", "Age: All", "Cycle: 2024ÔÇô2025", "Status: Active"].map(t => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <ChartCard title="NDVI Trend by Stand Age" subtitle="24-month canopy density ┬À Sentinel-2">
          ...
          <ChartCard title="Red-Edge CIre ┬À Nutrient Status" subtitle="Green = adequate ┬À Amber = deficient ┬À Red = severe">
          ...
          <ChartCard title="Disease Risk Monitor (BSR)" subtitle="Bubble = block area ┬À color = BSR risk">
            <ResponsiveContainer width="100%" height={240}>
              <ScatterChart margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)"/>
                <XAxis type="number" dataKey="age" name="Age" tick={{ fontSize: 10 }} unit="y" stroke="var(--color-muted-foreground)"/>
                <YAxis type="number" dataKey="anomaly" name="NDVI ╬ö" tick={{ fontSize: 10 }} stroke="var(--color-muted-foreground)"/>
          ...
          <ChartCard title="Rainfall vs Water Requirement" subtitle="CHIRPS monthly ┬À 150ÔÇô200mm/mo target" wide>
          ...
          <ChartCard title="Harvest Calendar" subtitle="52-week schedule ┬À gold = upcoming ┬À red = overdue" wide>
              ...
              {blocks.filter(b => b.age >= 5).map(b => {
                ...
                return (
                  <div key={b.id} className="flex items-center gap-2 text-[11px]">
                    ...
                    <span className="w-12 text-right font-mono text-muted-foreground">{overdue ? "ÔêÆ3w" : `+${(b.id.charCodeAt(2) % 5)}w`}</span>
                  </div>
                );
              })}
            </div>
