...
const METRICS = [
  "NDVI Canopy Health",
  "Red-Edge Chlorophyll (CIre) ÔÇö Nutrient Status",
  "FFB Yield Forecast",
...
const SAVED = [
  { name: "Q4 2025 Estate Summary ÔÇö Ondo Alpha", type: "Estate Summary", date: "Dec 1, 2025", status: "Final" },
  { name: "B07 Disease Risk Brief", type: "Disease Risk", date: "Nov 28, 2025", status: "Final" },
  { name: "Cross River FFB Forecast ÔÇö Q1 2026", type: "FFB Forecast", date: "Nov 22, 2025", status: "Draft" },
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr_1fr] gap-4 p-6 h-full">
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden flex flex-col">
        ...
        <div className="p-5 flex-1 overflow-auto">
          {step === 1 && (
            <Section title="Step 1 ÔÇö Select Scope">
              ...
              <div className="mt-4 max-h-64 overflow-auto border border-border rounded-md p-2 space-y-1">
                {blocks.map(b => (
                  <label key={b.id} className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-muted text-xs cursor-pointer">
                    ...
                    <span className="ml-auto text-muted-foreground">Age {b.age}y ┬À {b.areaHa}ha</span>
                  </label>
                ))}
              </div>
            </Section>
          ...
          {step === 2 && (
            <Section title="Step 2 ÔÇö Time Range">
          ...
          {step === 3 && (
            <Section title="Step 3 ÔÇö Metrics">
          ...
          {step === 4 && (
            <Section title="Step 4 ÔÇö Report Type">
              <div className="space-y-1.5">
                {REPORT_TYPES.map(t => (
          ...
          {step === 5 && (
            <Section title="Step 5 ÔÇö Export">
              <div className="grid grid-cols-2 gap-2">
              ...
              <p className="text-xs text-muted-foreground mt-3">Preview is shown on the right ÔåÆ</p>
            </Section>
          )}
      ...
      <div className="bg-card border border-border rounded-lg overflow-auto">
        <div className="px-5 py-4 border-b border-border bg-muted/30">
          ...
          <p className="text-[11px] text-muted-foreground mt-1">Ondo Alpha + Cross River Beta ┬À 6.5┬░N 5.5┬░E ┬À Generated Dec 4, 2025</p>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs text-foreground/90">
              <Insight><strong>B05</strong> ÔÇö Age 13y. Canopy excellent, FFB on track at 24 t/ha. Next harvest in 2 weeks.</Insight>
              <Insight><strong>B07</strong> ÔÇö NDVI decline detected (ÔêÆ0.19 anomaly). Possible early BSR infection. Recommend immediate field inspection.</Insight>
              <Insight><strong>B10</strong> ÔÇö Nutrient deficiency (CIre 0.8). Potassium application recommended before next wet season.</Insight>
              <Insight><strong>B11/B12</strong> ÔÇö Age 26y. Approaching replanting threshold. Yield declined to 11 t/ha.</Insight>
              <Insight><strong>Estate Total Projected FFB</strong>: ~{totals.ffbQuarter} t this quarter across 10 producing blocks.</Insight>
              <Insight><strong>Drought Alert</strong>: 3 blocks showing moisture stress. Rainfall deficit ÔêÆ58mm vs 10-year average.</Insight>
            </ul>
          </div>
        </div>
      </div>
...
function Insight({ children }: { children: React.ReactNode }) {
  return <li className="flex gap-2"><span className="text-primary mt-0.5">ÔÇó</span><span>{children}</span></li>;
}
