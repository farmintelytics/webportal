...
export function AIAssistant() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: "ai",
      content: "I'm analyzing **12 blocks** across both estates. Two blocks (**B11, B12**) are approaching replanting threshold (age 26y). Block **B07** is showing high BSR disease risk based on a ÔêÆ0.19 NDVI anomaly and red-edge decline over the last 4 months. Ask me anything, or run a scenario from the left panel.",
    },
  ]);
  ...
  const runScenario = () => {
    const summary = `Running **${scenario}** for ${estate} ┬À rainfall deviation ${rainDev > 0 ? "+" : ""}${rainDev}% ┬À harvest delay ${delay}wk ┬À fertilizer: ${fertilizer}`;
    ...
    <div className="flex h-full">
      ...
      <section className="flex-1 flex flex-col min-w-0">
        ...
        <div className="px-5 py-3 border-b border-border bg-card flex items-center gap-3 text-xs">
          ...
          <span className="font-medium truncate">{estate} ┬À {blocks.filter(b => b.estate === estate).length} blocks ┬À 2024ÔÇô2025 ┬À {Object.entries(layers).filter(([_, v]) => v).map(([k]) => k.split(" ")[0]).join(" + ")}</span>
        ...
        <div className="border-t border-border p-4 bg-card">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              placeholder="Ask about FFB forecasts, BSR risk, replanting, fertilizerÔÇª"
...
function generateResponse(prompt: string): Msg {
  ...
  if (p.includes("replant") || p.includes("aging")) {
    return {
      ...
      actions: [
        { tag: "High", text: "Stage replanting for B11 in Q1 ÔÇö soil prep & seedling order" },
        { tag: "Medium", text: "Schedule B12 6 months after to phase production loss" },
        { tag: "Low", text: "Re-survey adjacent blocks B09ÔÇôB10 next dry season" },
      ],
    };
  }
  if (p.includes("forecast") || p.includes("ffb")) {
    return {
      role: "ai",
      content: `Estimated **Q1 FFB production: ~${Math.round(blocks.reduce((s,b)=>s+b.ffbYield*b.areaHa,0)/4)} tonnes** across 10 producing blocks. Confidence interval ┬▒8% based on rainfall normality and current canopy state. Peak contributors: **B05, B06** (35ÔÇô40ha at 24 t/ha annualized).`,
      ...
      actions: [
        { tag: "High", text: "Confirm harvest crews for B05ÔÇôB06 peak window in week 4" },
  ...
  if (p.includes("bsr") || p.includes("disease")) {
    return {
      role: "ai",
      content: "**Block B07** is showing strong BSR disease signatures:\n\nÔÇó NDVI dropped from 0.71 ÔåÆ 0.49 over 4 months (anomaly ÔêÆ0.19)\nÔÇó Red-edge CIre declining in localized cluster\nÔÇó Adjacent **B08** showing early-stage decline (CIre 1.1)\n\nGanoderma BSR typically presents this exact remote-sensing fingerprint 6ÔÇô12 months before visible canopy collapse.",
      actions: [
        { tag: "High", text: "Dispatch field team to B07 for soil sampling and basal palm inspection" },
        { tag: "High", text: "Quarantine B07 ÔÇö halt harvest sharing equipment with healthy blocks" },
  ...
  if (p.includes("delay") || p.includes("harvest")) {
    return {
      role: "ai",
      content: "Delaying harvest by **2 weeks** across the estate would result in:\n\nÔÇó ~**3.2% FFB weight gain** from late ripening\nÔÇó ~**11% loss** from over-ripeness, oil quality degradation, and bruising\nÔÇó Net effect: **ÔêÆ7.8% revenue** at current CPO prices\n\nHowever, delaying B07 specifically (currently overdue) is necessary pending BSR inspection.",
      chart: [
  ...
  if (p.includes("underperform") || p.includes("top 3")) {
    return {
      role: "ai",
      content: "Top 3 underperforming producing blocks vs estate average (18.2 t/ha):\n\nÔÇó **B11** ÔÇö 11.0 t/ha (age 26y, structural decline, replant candidate)\nÔÇó **B12** ÔÇö 11.2 t/ha (age 26y, replant candidate)\nÔÇó **B10** ÔÇö 16.0 t/ha (age 22y, severe nutrient stress CIre 0.8)\n\nB11/B12 are age-driven; B10 is reversible with K/N intervention.",
      chart: [
        { label: "B11", value: 11, color: "var(--color-replant)" },
        { label: "B12", value: 11.2, color: "var(--color-replant)" },
  ...
  return {
    role: "ai",
    content: `Analysis ready for **${prompt}**. Across 12 blocks, current canopy health is mostly stable ÔÇö peak production blocks B05/B06 at NDVI 0.74. Active concerns: B07 (BSR risk), B10 (nutrient stress), B11/B12 (replanting age).`,
    actions: [
