...
import { Plot, plots, alertColors, stageColors } from "@/lib/mockData";
import {
  Layers, Droplets, Sprout, Activity, X, ChevronRight, ChevronLeft, MapPin,
          ...
          {open ? (
            <div className="bg-card/95 backdrop-blur border border-border rounded-xl shadow-glow max-h-[calc(100vh-220px)] overflow-y-auto">
              ...
              <div className="px-4 py-3 border-b border-border">
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Basemap</div>
                <div className="grid grid-cols-3 gap-1.5">
                  {(["satellite","street","terrain"] as const).map(b => (
                    <button key={b} onClick={() => setBasemap(b)}
                      className={`text-xs py-1.5 rounded-md border capitalize transition ${
                        basemap === b ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-accent"
                      }`}>{b}</button>
                  ))}
                </div>
              </div>
